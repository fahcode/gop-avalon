            ...
            im.ajaxget(flow, CONFIG.AJAX_URL.workView, 'GET', 'json', {flow_order_id: fid}, function(data) {
                ...
                for(var i in threads){
                    ...
                    if(dd.node_flag=="node_cmp_provider"){
                        flow.node_cmp_provider = threads[i-1].data;
                        /////已经提交报价 但是节点未走过
                        if(dd.data[0].replys.length>0){
                            ...
                            flow.baojia_result.node_name = "报价";
                            flow.baojia_result.status = -1;
                        }
                    }
                    ///有正常流程清除老的流程
                    if(dd.node_flag=="node_cmp_reply"){
                        flow.showStates.baojia_result = false;
                        flow.baojia_result = {}
                    };
                };
