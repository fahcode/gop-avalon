...
define(['swiper', 'webUploader', 'calendar', '$md5', "sly"], function(swiper, WebUploader) {
    var flow = avalon.define({
        ...
        events: {
            ...
            submitFlow: function(callback, type){
                ...
                if(flow.showStates.baojia){
                    ...
                    callback = function(){
                        ...
                        flow.allNode[flow.allNode.length-1].isOrder = false;
                        //手动添加数据
                        flow.baojia_result = {
                            status: -1,
                ...
                if(flow.showStates.work_analysis){
                    ...
                    ops = {
                        ...
                        fdate: flow.editData.work_analysis.fdate,
                    ...
                    callback = function(){
                        ...
                        flow.allNode[flow.allNode.length-1].isOrder = false;
                        flow.allNode[flow.allNode.length-1].data = {
                            fdate: flow.editData.work_analysis.fdate,
                            content: flow.editData.work_analysis.content,
            ...
            showLastBtn: function(){
                //结束的节点就不显示了
                if(flow.lastNodeName=="finish"||flow.lastNodeName=="close") return false;

                var allBtns = $('.nodeboxs>div').find('.nodebtn'),
                    lastBtn = allBtns.eq(allBtns.length-1),
                    nextAllBtn = lastBtn.nextAll();

                console.log(lastBtn);
                //当前按钮去掉关闭
                lastBtn.removeClass('close');
                //按钮后面的全部显示
                nextAllBtn.show();
                //编辑默认就是显示的，所以不用处理
            }
    ...
    return avalon.controller(function($ctrl) {
        ...
        $ctrl.$onRendered = function(params, rs) {
            ...
            $("#calendar2").Calendar({time: true, onlyYM: false});
            //报价上传
            ////上传相关附件
        ...
        $ctrl.$onEnter = function(params, rs) {
            ...
            flow.editData.work_submit.stype = !!ftype? ftype: 1;
            ...
            im.ajaxget(flow, CONFIG.AJAX_URL.workView, 'GET', 'json', {flow_order_id: fid}, function(data) {
                ...
                for(var i = 0,c = 0;i<cpnodes.length;i++){
                    ...
                    if(dd.node_flag=="node_cmp_provider"){
                        ...
                        if(dd.data[0].replys.length>0){
                            ...
                            flow.baojia_result.status = dd.data[0].status;
                        }
                    }
                ...
                if(lastNodeName=="finish"||lastNodeName=="close"){
                    order.title = null;
                    allNode.push(order);
                }
